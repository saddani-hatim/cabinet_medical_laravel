        ...
        <div class="relative min-h-screen overflow-hidden">
            ...
            <nav class="max-w-7xl mx-auto px-6 py-8 flex items-center justify-between relative z-10">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xl font-bold tracking-tight text-slate-900">{{ config('app.name', 'Cabinet Médical') }}</span>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="btn-premium bg-emerald-500 text-white hover:bg-emerald-600">{{ __('Dashboard') }}</a>
                        @else
                            <a href="{{ route('login') }}" class="text-sm font-semibold text-slate-700 hover:text-emerald-600 transition-colors">{{ __('Login') }}</a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="btn-premium bg-white text-slate-900 border border-emerald-200 hover:bg-emerald-50">{{ __('Register') }}</a>
                            @endif
                        @endauth
                    @endif
                </div>
                ...
                <div class="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div class="order-1 lg:order-1 relative">
                        ...
                        <div class="premium-card p-8 bg-white shadow-2xl relative overflow-hidden group">
                            ...
                            <div class="flex items-center justify-between mb-10">
                                <h3 class="text-xl font-bold text-slate-900">{{ __('Upcoming Appointments') }}</h3>
                            ...
                            <div class="space-y-6 relative z-10">
                                <div class="flex items-center gap-4 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 hover:shadow-md transition-shadow cursor-pointer hover:-translate-y-1 transform duration-200">
                                    ...
                                    <div>
                                        <span class="block text-xs font-bold text-emerald-500 uppercase tracking-wider">{{ __('General Practice') }}</span>
                                        <span class="block text-slate-900 font-bold">{{ __('Today, 2:00 PM') }}</span>
                                    </div>
                                </div>
                                <div class="flex items-center gap-4 p-4 rounded-2xl bg-teal-50 border border-teal-100 hover:shadow-md transition-shadow cursor-pointer hover:-translate-y-1 transform duration-200">
                                    ...
                                    <div>
                                        ...
                                        <span class="block text-slate-900 font-bold">{{ __('Tomorrow, 11:00 AM') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-10 pt-8 border-t border-emerald-50 flex items-center justify-between">
                                ...
                                <span class="text-sm font-bold text-emerald-600">{{ __('+24 this week') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="absolute -right-8 bottom-1/4 animate-float hidden md:flex items-center gap-3 bg-white p-3 rounded-2xl shadow-xl border border-emerald-100 z-20">
                            ...
                            <div>
                                <span class="block text-xs font-bold text-emerald-600 uppercase">{{ __('Status') }}</span>
                                <span class="block text-sm font-bold text-slate-800">{{ __('Doctors Available Now') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="order-2 lg:order-2 lg:pl-10">
                        <span class="inline-block px-4 py-1.5 rounded-full bg-emerald-100 text-emerald-700 text-sm font-bold tracking-wide uppercase mb-6 shadow-sm border border-emerald-200">
                            ✨ {{ __('Excellence in Healthcare') }}
                        </span>
                        <h1 class="text-5xl lg:text-7xl font-extrabold text-slate-900 leading-[1.1] mb-8">
                            {{ __('Your Health, Our Priority') }} <br><span class="text-gradient">{{ __('Advanced Medical Care') }}</span> <br>{{ __('at Your Fingertips') }}
                        </h1>
                        <p class="text-lg text-slate-600 mb-10 leading-relaxed max-w-lg">
                            {{ __('Experience cutting-edge healthcare with our expert team. Book appointments, access records, and manage your wellness journey seamlessly.') }}
                        </p>
                        <div class="flex flex-col xl:flex-row gap-4">
                            ...
                            <div class="flex items-center gap-4 px-4 py-2 border border-emerald-200 rounded-xl bg-white/70 backdrop-blur-sm self-start md:self-auto">
                                <div class="flex -space-x-2">
                                    <img class="w-8 h-8 rounded-full border-2 border-white" src="https://ui-avatars.com/api/?name=Dr+John&background=10b981&color=fff" alt="Doctor">
                                    <img class="w-8 h-8 rounded-full border-2 border-white" src="https://ui-avatars.com/api/?name=Dr+Jane&background=059669&color=fff" alt="Doctor">
                                    <img class="w-8 h-8 rounded-full border-2 border-white" src="https://ui-avatars.com/api/?name=Dr+Smith&background=047857&color=fff" alt="Doctor">
                                </div>
                                <div class="text-xs">
            ...
            <section class="py-24 bg-white relative border-y border-slate-100">
                <div class="max-w-7xl mx-auto px-6 relative z-10">
                    <div class="flex flex-col md:flex-row items-end justify-between mb-16 gap-6">
                        <div>
                            <span class="text-emerald-500 font-bold tracking-wider uppercase text-sm mb-2 block">{{ __('Simple Process') }}</span>
                            <h2 class="text-4xl font-extrabold text-slate-900 mb-4">{{ __('How It Works') }}</h2>
                            <p class="text-lg text-slate-600 max-w-xl">{{ __('Getting the care you need is simple. Follow these easy steps to start your healthcare journey with us.') }}</p>
                        </div>
                    ...
                    <div class="grid md:grid-cols-3 gap-8 relative mt-12">
                        ...
                        <div class="text-center group">
                            ...
                            <h3 class="text-xl font-bold text-slate-900 mb-4">{{ __('Create Your Account') }}</h3>
                            <p class="text-slate-600">{{ __('Sign up in seconds with your basic information. No lengthy forms or waiting periods.') }}</p>
                        </div>
                        <div class="text-center group">
                            ...
                            <h3 class="text-xl font-bold text-slate-900 mb-4">{{ __('Book Your Appointment') }}</h3>
                            <p class="text-slate-600">{{ __('Choose your preferred doctor, date, and time. Get instant confirmation and reminders.') }}</p>
                        </div>
                        <div class="text-center group">
                            ...
                            <h3 class="text-xl font-bold text-slate-900 mb-4">{{ __('Receive Quality Care') }}</h3>
                            <p class="text-slate-600">{{ __('Arrive for your appointment and receive personalized, expert medical attention.') }}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="unique-section py-24 relative overflow-hidden">
                ...
                <div class="max-w-7xl mx-auto px-6 relative z-10">
                    <div class="text-center mb-16">
                        <span class="text-emerald-600 font-bold tracking-wider uppercase text-sm mb-2 block">{{ __('Our Advantages') }}</span>
                        <h2 class="text-4xl font-extrabold text-slate-900 mb-4">{{ __('Why Choose Our Cabinet?') }}</h2>
                        <p class="text-lg text-slate-600 max-w-2xl mx-auto">{{ __('Discover what sets our medical practice apart and why patients trust us with their healthcare needs.') }}</p>
                    </div>
                    <div class="grid md:grid-cols-3 gap-8">
                        <div class="text-center premium-card p-8 hover:-translate-y-2 transition-transform duration-300 bg-white/80 backdrop-blur-md">
                            ...
                            <h3 class="text-xl font-bold text-slate-900 mb-4">{{ __('Lightning Fast') }}</h3>
                            <p class="text-slate-600">{{ __('Get appointments scheduled in minutes, not days. Our streamlined system ensures you see the right doctor at the right time.') }}</p>
                        </div>
                        <div class="text-center premium-card p-8 hover:-translate-y-2 transition-transform duration-300 bg-white/80 backdrop-blur-md">
                            <div class="w-16 h-16 medical-gradient rounded-2xl flex items-center justify-center text-white mx-auto mb-6 shadow-lg shadow-emerald-200">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
                            </div>
                            <h3 class="text-xl font-bold text-slate-900 mb-4">{{ __('Trusted Security') }}</h3>
                            <p class="text-slate-600">{{ __('Your health information is protected with bank-level security. Privacy and confidentiality are our top priorities.') }}</p>
                        </div>
                        <div class="text-center premium-card p-8 hover:-translate-y-2 transition-transform duration-300 bg-white/80 backdrop-blur-md">
                            <div class="w-16 h-16 medical-gradient rounded-2xl flex items-center justify-center text-white mx-auto mb-6 shadow-lg shadow-emerald-200">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                            </div>
                            <h3 class="text-xl font-bold text-slate-900 mb-4">{{ __('Experienced Team') }}</h3>
                            <p class="text-slate-600">{{ __('Our team of highly qualified healthcare professionals brings years of experience and expertise to provide the best possible care.') }}</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <footer class="bg-slate-900 text-slate-300 py-12 relative overflow-hidden">
                ...
                <div class="max-w-7xl mx-auto px-6 relative z-10 text-center">
                    <div class="flex items-center justify-center gap-2 mb-6">
                        ...
                        <span class="text-xl font-bold tracking-tight text-white">{{ config('app.name', 'Cabinet Médical') }}</span>
                    </div>
